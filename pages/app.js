...
function App({ userBots }) {
	...
	const removeBot = () => {
		if (bots.length !== 1) {
			...
			const newState = [...bots.slice(0, currentBotIndex), ...bots.slice(currentBotIndex + 1)]
	...
	const addGroup = () => {
		...
		const newState = [...bots.slice(0, currentBotIndex), oldBot, ...bots.slice(currentBotIndex + 1)]
